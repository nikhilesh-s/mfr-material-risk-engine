...
function RankingPage() {
  ...
  return (
    <PageContainer eyebrow="Ranking" title="Candidate material ranking" description="Rank a shortlist of candidate materials and export the leaderboard for testing workflows.">
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <MaterialCard title="Ranking input table" subtitle="One material name per line. Demo values are preloaded.">
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={runRanking} className="rounded-full bg-[var(--dravix-gradient-primary)] px-4 py-2 text-sm text-white">Run ranking</button>
          ...
          {exportContent ? <pre className="mt-4 max-h-48 overflow-auto rounded-2xl bg-[var(--dravix-panel)] p-4 text-xs">{exportContent}</pre> : null}
        </MaterialCard>
      ...
      <MaterialCard title="Ranked materials">
        <div className="space-y-3">
          {(result?.ranking ?? []).map((row) => (
            <div key={row.rank} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--dravix-border)] p-4">
              <div>

...
function DatasetUploadForm({ onSubmit, loading = false }: Props) {
  ...
  return (
    <form onSubmit={submit} className="rounded-[1.5rem] border border-[var(--dravix-border)] bg-white p-5 shadow-[var(--dravix-shadow-soft)]">
      ...
      <input type="file" name="dataset_file" accept=".csv" className="mt-4 block w-full text-sm" />
      <button type="submit" disabled={loading} className="mt-4 rounded-full bg-[var(--dravix-gradient-primary)] px-4 py-2 text-sm text-white disabled:opacity-60">
